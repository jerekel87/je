...
function SectionTwo({ article }: { article: Article }) {
  return (
    <section className="lg:px-[58px] mt-[50px] lg:mt-[65px]">
      {article.block?.title && (
        <h2 className="font-portlin tracking-[0.5px] text-[30px] lg:text-[60px] leading-[.9]">
          {article.block.title}
        </h2>
      )}
      {article.block?.body && (
        <PortableText
          ...
          className="mt-6 lg:mt-[40px] text-muted-foreground leading-normal text-sm lg:text-lg"
        />
      )}
